...
--------------------------
Core library for connecting to T-Invest API and extracting portfolio data.

All other scripts import from here — never instantiate API clients directly.
...
# Instrument types included in portfolio analysis (excludes cash/derivatives)
...
def get_instrument_info(client: Client, figi: str) -> Optional[Dict]:
    ...
    Returns None for unknown/derivative instruments — callers must skip those.
    Reclassifies gold/metals to 'precious_metal' regardless of API response.
    """
    if not figi or not figi.strip():
        logger.debug("Skipping position with empty FIGI")
        return None
    try:
        ...
        for inst in resp.instruments:
            ...
            if itype == "unknown":
                return None  # derivative — caller skips
...
def fetch_all_positions(
    ...
    Each dict contains: account, figi, name, ticker, currency,
    instrument_type, quantity, avg_price, cur_price, rub_value.

    Skips:
    - derivative/unknown instruments (instrument_type == 'unknown')
    ...
    for account in accounts:
        ...
        for pos in portfolio.positions:
            ...
            # Option heuristic: unknown name + negative quantity
...
# ── Historical prices (async internals) ───────────────────────────────────────
...
def fetch_price_history(
    ...
    """
    Fetch daily close price history for a set of instruments.
...
def portfolio_weights(
    ...
    """
    Compute RUB weight (%) per ticker for the given ticker list.

    Positions not in `tickers` are ignored.
    """
    rub: Dict[str, float] = {}
    for p in positions:
        t = p.get("ticker", "")
        if t in tickers:
            rub[t] = rub.get(t, 0.0) + float(p.get("rub_value") or 0)
    total = sum(rub.values())
    if total <= 0:
        return pd.Series(0.0, index=tickers)
    return pd.Series(
        {t: rub.get(t, 0.0) / total * 100 for t in tickers},
        name="weight_pct",
    )
